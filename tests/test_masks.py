...
def input_data() -> list[str]:
    return [
        ...
        "—чет 64686473678894779589",
        "MasterCard 7158300734726758",
        "—чет 35383033474447895560",
...
def test_get_uncorrect_name_card_number(input_data: str) -> Any:
    with pytest.raises(ValueError):
        get_mask_card_number("—чет 73654108430135874305")
...
def user_data() -> list[str]:
    return ["—чет 64686473678894779589",
            "—чет 35383033474447895560",
            "Visa Platinum 8990922113665229",
            "Visa Gold 5999414228426353",
            "—чет 73654108430135874305",
...
@pytest.mark.parametrize("user_data, expected_mask", [
                        ("—чет 64686473678894779589", "—чет **9589"),
                        ("—чет 35383033474447895560", "—чет **5560"),
                        ("—чет 73654108430135874305", "—чет **4305"),
                        ])
